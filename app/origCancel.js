...
function createOrigEC(freq) {
  ...
  ec.renderBuffer={
    buffer: new Float32Array(2448), // 2448=19*128, aec3. decimateされた信号を逆順に格納するリングバッファ
    write: 2432, // 2448-16 aec3.
    read: 2432, // 最初に書いたときに writeが2416になり、そのときに同じ値になるようにしておく。
    copyBlock: function(sbReversed) {
      if(sbReversed.length!=16) throw "invalid_size";
      for(let i=0;i<16;i++) this.buffer[this.write+i]=sbReversed[i];
      this.write-=16;  // 逆順リングバッファなので前に移動させていく。
      if(this.write==-16) this.write=2432; // 一周する
    },
    dumpBuffer: function() {
      console.log("renderBuffer dump: read:",this.read,"write:",this.write,"size:",this.buffer.length);
      console.log(this.buffer.join(","));
    },
    incrementReadIndex: function() {
      this.read-=16;
      if(this.read==-16) this.read=2432; // 一周
    }
  };
  ...
  ec.process = function(i16out) {
    ...
    if(version==0) {
      ...
      for(let bi=0;bi<blockNum;bi++) {
        ...
        ec.renderBuffer.copyBlock(refLowBlockReversed);
        ec.renderBuffer.dumpBuffer();
        ec.renderBuffer.incrementReadIndex();
        ...
        for(let n=0;n<ec.filters.length;n++) {
          ...
          let xStartIndex = (ec.renderBuffer.read + alignmentShift + subBlockSize - 1 ) % ec.renderBuffer.buffer.length;
          console.log("calling matchedFilterCore xStartIndex:",xStartIndex,"n:",n);
          const smoothing=0.7;
          const y=recLowBlock;
          const coreOut=matchedFilterCore(xStartIndex,x2SumThreshold,smoothing,ec.renderBuffer.buffer,y,ec.filters[n]);
          ...
          if(coreOut.filterUpdated && reliable && coreOut.errorSum < winnerErrorSum) {
            ...
            console.log("winner updated. n:",n,"lag:",lag);
          }
        ...
        if(winnerIndex!=-1) {
          ...
          console.log("winnerLag:",winnerLag);
        }
